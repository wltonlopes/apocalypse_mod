...
var defendTime = () => randFloat(25, 25);//25minutes long


Trigger.prototype.IntroductionMessage = function(data) 
{
	var output = ["Welcome!", "A group of raiders are around the region" , " prosper our city and fortify it as quickly as possible!","My Ajaw une fortress is very preciosos in we posision!"];
...
{
	let infZumb = ["units/zumb/zumb_infantry","units/zumb/zumb_infantry"]
	var intruders = TriggerHelper.SpawnUnitsFromTriggerPoints(
			pickRandom(["B"]),pickRandom(infZumb), 3, 0);
...
Trigger.prototype.Reforces = function()
{
	var alieds = TriggerHelper.SpawnUnitsFromTriggerPoints(
			pickRandom(["A"]),"units/surv/support_citizen",5, 1);
	// var alieds = TriggerHelper.SpawnUnitsFromTriggerPoints(
...
cmpTrigger.DoAfterDelay(50000, "Reforces", {});
